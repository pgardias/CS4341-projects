...
class Expectimax4(CharacterEntity):
	origin = 0, 0
	firstTurn = True
		...
		for i in range(wrld.width()):
			for j in range(wrld.height()):
				...
				if (wrld.bomb_at(i, j) or sim.bomb_at(i, j)):
					...
					if (bomb.timer <= 1 or futurebomb.timer <= 1):
						return True
		return False
	...
	def explosionNextTurn(self, move, wrld):
		...
		if (wrld.explosion_at(startpos[0] + move[0], startpos[1] + move[1])) or wrld.explosion_at(startpos[0],
		                                                                                          startpos[1]):
			return True
	...
	def expectimaxBestMove(self, wrld):
		...
		for x in myMoves:
			if x is None:
				return self.getMoveFromPath(self.bombastar(wrld, me, self.origin), self.origin)
			test = wrld.from_world(wrld)
			character = test.me(self)
			character.move(x[0], x[1])
			dead = test.next()[0]
			if (self.explosionNextTurn(x, dead) or self.explosionNextTurn(x, wrld)):
				# print("badMove: " , x)
				z = 1  # do nothing line
			else:
				safeMoves.append(x)
				...
				for a2 in newSafeMoves:
					...
					for m2 in finalMonsterMoves:
						...
						mySecondsum += 3 * self.distance(a2, m2) + -2 * self.distance(a2,
						                                                              wrld.exitcell) + -10000 * self.explosionNextTurn(
							a2, nextState)
	...
	def do(self, wrld):
		...
		if (monsterDistance is not None):
			if (len(self.astar(wrld, me, monster)) <= 4) and monster[1] >= me[
		...
		if self.distance(me, wrld.exitcell) < self.distance(newme, wrld.exitcell) and not self.monsterDead(
				wrld) and len(monsterDistance) <= 3:
			self.place_bomb()
		self.move(move[0], move[1])
