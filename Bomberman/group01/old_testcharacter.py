...
class TestCharacter(CharacterEntity):
	...
	def best_action(self, simulating, wrld):
		...
		next_to_explosion = False
		if best_action == (0, 0) and not self.waiting_for_explosion:
			for move in self.get_empty_spaces((self.x, self.y), wrld):
				if wrld.explosion_at(self.x + move[0], self.y + move[1]) is not None:
					next_to_explosion = True
			if not next_to_explosion:
				self.place_bomb()
				self.waiting_for_explosion = True
				self.turns_until_explosion_ends = self.duration_bomb_and_exp
	...
	def distance_to_monster(self, pos, wrld):
		closest_monster, monster_found = self.find_closest_monster(pos, wrld)
		if monster_found:
			...
			if shortest_distance_to_monster <= self.max_distance_to_detect_monster:
				avg = (shortest_distance_to_monster + self.distance(closest_monster, pos)) / 2
				f = 1 - avg / self.max_distance_to_detect_monster
				if f < 0:
					return 0
				else:
					return f
