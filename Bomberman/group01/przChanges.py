...
class TestCharacter(CharacterEntity):
   ...
   gamma = 0.9
   learning_rate = 0.2
   max_distance_to_detect_monster = 5
   ...
   def bestAction(self, wrld):
      ...
      for action in self.getValidMoves((wrld.me(self).x, wrld.me(self).y), wrld):
         """Features for qval:
         ...
         next_position = (wrld.me(self).x + action[0], wrld.me(self).y + action[1])
   ...
   def predictAggressiveMonsterMove(self, pos, wrld):
      ...
      for move in monster_valid_moves:
         path = self.astar(wrld, (pos[0] + move[0], pos[1] + move[1]), (wrld.me(self).x, wrld.me(self).y))
         if path is not None:
            distance = len(path)
   ...
   def distanceToMonster(self, action, wrld):
      ...
      # get A* path length for each possible monster position
      monster_found = False
      ...
      for monster_pos in monster_positions:
         if self.monster_moves_randomly:
            ...
            path = self.astar(wrld, (wrld.me(self).x + action[0], wrld.me(self).x + action[1]),
                              (monster_pos[0] + monster_move[0], monster_pos[1] + monster_move[1]))
            if path is not None:
               monster_found = True
   ...
   def explosionNextTurn(self, move, wrld):
      ...
      startpos = wrld.me(self).x, wrld.me(self).y
      character.x = wrld.me(self).x + move[0]
      character.y = wrld.me(self).y + move[1]
      sooner = wrld.next()
      for e in sooner[1]:
         if e.tpe == 2:
            return True
   ...
   def nextWorldState(self, action, wrld):
      ...
      for m in future.monsters().values():
         if self.monster_moves_randomly:
            print('TODO')  # TODO
         else:
            future.update_monster_move(m, self.predictAggressiveMonsterMove())

      # increment turn
      next_world, events = future.next()
      ...
      for i in [-1, 0, 1]:
         for j in [-1, 0, 1]:
            ...
            if 0 <= x < wrld.width() and 0 <= y < wrld.height():
               if wrld.empty_at(x, y) or wrld.explosion_at(x, y) or wrld.exit_at(x, y) or wrld.monsters_at(x, y):
                  move = x, y
