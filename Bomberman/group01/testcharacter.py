...
class TestCharacter(CharacterEntity):
	...
	def distanceToMonster(self, action, wrld):
		...
		for location in monster_locations:
			path = self.astar(wrld, (self.x + action[0], self.y + action[1]), location)
			if path is not None:
	...
	#
	# 		if current[0] == end: # condition is never met
	# 			path = []
	# 			current = current[0]
...
# new astar is not returning an actual path, instead finding costs to all cells. needs to actually construct a path using
# this information, see the following for poor implementation:
# https://www.redblobgames.com/pathfinding/a-star/implementation.html#python-astar
	...
	def astar(self, wrld, start, end):
		...
		while len(frontier) != 0:
			print(frontier)
			frontier.sort(key=self.getPriority)
			current = frontier[0]
			...
			for next in self.getValidSpotsFrom(current[0], wrld):
				newCost = costSoFar[current[0]] + 1
		...
		for i in [-1, 0, 1]:
			for j in [-1, 0, 1]:
				...
				if 0 <= pos[0] < wrld.width() and 0 <= pos[1] < wrld.height():
					if wrld.empty_at(pos[0], pos[1]) or wrld.monsters_at(pos[0], pos[1]) or wrld.exit_at(pos[0], pos[
						1]) or wrld.characters_at(pos[0], pos[1]):
