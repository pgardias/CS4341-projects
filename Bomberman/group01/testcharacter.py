...
class TestCharacter(CharacterEntity):
   ...
   def genNewWeights(self, action, qval, wrld):
      ...
      for weight in self.w:
         new_weight = 0
         if feature is 0:
            new_weight = weight + self.learningRate * (wrld.scores['me'] + gamma * next_qval - qval) * self.distanceToExit(action, wrld)
            print("Weight of distanceToExit: ", self.distanceToExit(action, wrld))
         elif feature is 1:
            new_weight = weight + self.learningRate * (wrld.scores['me'] + gamma * next_qval - qval) * self.distanceToMonster(action, wrld)
            print("Weight of distanceToMonster: ", self.distanceToMonster(action, wrld))
         elif feature is 2:
            new_weight = weight + self.learningRate * (wrld.scores['me'] + gamma * next_qval - qval) * self.InExplosion(wrld, action)
