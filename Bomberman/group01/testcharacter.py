	...
	def distanceToMonster(self, action, wrld):
		...
		for monster_pos in monster_positions:
			if self.monster_moves_randomly:
				...
				path = self.astar(wrld, (self.x + action[0], self.y + action[1]),
		...
		if monster_found:
			if len(path) <= self.max_distance_to_detect_monster:
				return len(path) / self.max_distance_to_detect_monster
			else:
				return 0
		else:
			return 0
